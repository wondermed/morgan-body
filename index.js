...
function getThemeObj(themeName) {
	...
	themes.noColorsTheme = Object.keys(themes.defaultTheme).reduce(
		(prev, curr) => {
			prev[curr] = "";
			return prev;
		},
...
function bodyToString(
	...
	if (isString && body.length) {
		...
		if (body.length > maxBodyLength)
			body = body.slice(0, maxBodyLength) + separatorStr + "...";
		...
		if (stringifiedObj.length > maxBodyLength)
			stringifiedObj =
				stringifiedObj.slice(0, maxBodyLength) + separatorStr + "...";
		...
		for (let i = 0; i < splitOnNewLineLength; i++) {
			finalStr +=
				lineSeparator + bodyColor + splitOnNewLine[i] + defaultColor;
		}
	}
...
module.exports = function morganBody(app, options) {
	...
	var timezone = options.hasOwnProperty("timezone")
		...
		: "local";
	var noColors = options.hasOwnProperty("noColors")
		? options.noColors
		: false;
	var prettify = options.hasOwnProperty("prettify") ? options.prettify : true;
	var includeNewLine = options.hasOwnProperty("includeNewLine")
		? options.includeNewLine
		: prettify;
	var includeFinalNewLine = options.hasOwnProperty("includeFinalNewLine")
		? options.includeFinalNewLine
		: false;
	var filterParameters = options.hasOwnProperty("filterParameters")
		? options.filterParameters
		: [];
	var immediateReqLog = options.hasOwnProperty("immediateReqLog")
		? options.immediateReqLog
		: false;
	...
	morgan.format(
		...
		function developmentFormatLine(tokens, req, res) {
			...
			if (!fn) {
				...
				if (logReqDateTime)
					formatString +=
						" " + userAgentColor + "at " + dateColor + ":date";
		...
		if (logResponseBody) {
			...
			app.use(
				morgan(
					logBodyGen(
						"Response",
						(req, res) => res[morganBodyResponseSymbol]
					),
					morganOptions
				)
			);
	...
	morgan.format(
		...
		function developmentFormatLine(tokens, req, res) {
			...
			if (!fn) {
				...
				if (logAllResHeader) {
					formatString +=
						" headers[:response-headers]" + defaultColor;
				} else {
...
function morgan(format, opts) {
	...
	if (buffer) {
		// flush interval
		var interval =
			typeof buffer !== "number" ? defaultBufferDuration : buffer;
